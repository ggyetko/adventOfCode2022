...
def testAllSuborders(currentOrder, remainingValves, timeRemaining, elephantOrder):
    global mostFlow
    doAnyFit = False
    for r in remainingValves:
        ...
        if timeRemaining > timeToReachR:
            ...
            forward.remove (r)
            testAllSuborders(currentOrder + [r], forward, timeRemaining - timeToReachR, elephantOrder)
    if elephantOrder == None:
        testAllSuborders(["AA"], remainingValves, 26, currentOrder)
    else:
        thisFlow = runTrial(currentOrder[1:], elephantOrder[1:])
        if thisFlow > mostFlow:
            mostFlow = thisFlow
            print ("Highest Flow so far", mostFlow, currentOrder, elephantOrder)
...
testAllSuborders(["AA"], valveFlowList, 26, None)
