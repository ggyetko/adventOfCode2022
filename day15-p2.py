...
class SparseRange:
    ...
    def findGap(self):
        if len(self.ranges) == 2:
            if self.ranges[1][0] - self.ranges[0][1] == 2:
...
for line in file:
    ...
    beaconList.append((data[2],data[3]))

for y in range (0,4000000):
    scannableSpots = grid.getNoBeaconRange(y)
    gap = scannableSpots.findGap()
    if y % 100000 == 0:
        print ("Row:",y)
    if gap != None and gap >=0 and gap <= 4000000 and (gap,y) not in beaconList:
        print ("Gap found in Row #:",y," Column#:",gap)
        print ("Tuning Frequency:",gap*4000000+y)
        break
